  ...
  if strcmp(ext, '.txt')
    ...
    while true
      ...
      if ~ischar(line)
        ...
        if isempty(line)
          isInfoSection = false;
        elseif regexpi(line, 'Block	Cycle	Trial	Target	Color	RT	Accuracy');
          isInfoSection = false;
        else
          if ~isInfoSection 
            isDataSection = true;
        ...
        if isDataSection
          [blockName, cycleNo, trialNo, target, color, rt, accuracy] = strread(line, '%s %d %d %s %s %f %c');
          ...
          if strcmp(blockName, 'ExpB')
            ...
            exp.subject(count).block(blockNo).cycle(cycleNo).trial(trialNo).color = color;
