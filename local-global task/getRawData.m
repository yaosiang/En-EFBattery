  ...
  if strcmp(ext, '.txt')
    ...
    while true
      ...
      if ~ischar(line)
        ...
        if isempty(line)
          isInfoSection = false;
        elseif regexpi(line, 'Block	Cycle	Trial	Target	RT	Accuracy');
          isInfoSection = false;
        else
          if ~isInfoSection 
            isDataSection = true;
        ...
        if isDataSection
          [blockName, cycleNo, trialNo, target, rt, accuracy] = strread(line, '%s %d %d %s %f %c');

          if strcmp(blockName, 'ExpB')
            blockNo = 1;
            exp.subject(count).block(blockNo).name = blockName;
            exp.subject(count).block(blockNo).cycle(cycleNo).trial(trialNo).targetFilename = target;
