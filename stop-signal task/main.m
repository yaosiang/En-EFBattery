...
try
    ...
    % Do the experiment:
    [parms, result{1}] = doBlock('Build Practice', windowPtr, parms);
    % Show question message again:
    showCenteredMessage(windowPtr, parms.question2Msg, parms.foreColor);
    getResponseRT(spaceKey);
    
    [parms, result{2}] = doBlock('Build', windowPtr, parms);
    
    showInstruction(windowPtr, 2, 1, parms.foreColor);
    [parms, result{3}] = doBlock('Stop Practice', windowPtr, parms);
    % Show question message again:
    showCenteredMessage(windowPtr, parms.question2Msg, parms.foreColor);
    getResponseRT(spaceKey);
    
    [parms, result{4}] = doBlock('Stop', windowPtr, parms);
    rt = toc;
    ...
    fprintf(fid, 'Build Block Mean RT: %f\n', parms.buildBlockMeanRT);
    fprintf(fid, 'Stop Signal Onset Time: %f\n\n', parms.stopSignalOnsetTime);
