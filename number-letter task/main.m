...
try
  ...
  % Do the experiment:
  result{1} = doBlock('Number Practice', windowPtr, parms);
  % Show question message again:
  showCenteredMessage(windowPtr, parms.question2Msg, parms.foreColor);
  getResponseRT(spaceKey);

  result{2} = doBlock('Number', windowPtr, parms);
  
  showInstruction(windowPtr, 2, 1, parms.foreColor);
  result{3} = doBlock('Letter Practice', windowPtr, parms);
  % Show question message again:
  showCenteredMessage(windowPtr, parms.question2Msg, parms.foreColor);
  getResponseRT(spaceKey);
  
  result{4} = doBlock('Letter', windowPtr, parms);
  
  showInstruction(windowPtr, 3, 1, parms.foreColor);
  result{5} = doBlock('Number-Letter Practice', windowPtr, parms);
  % Show question message again:
  showCenteredMessage(windowPtr, parms.question2Msg, parms.foreColor);
  getResponseRT(spaceKey);
  
  result{6} = doBlock('Number-Letter', windowPtr, parms);
  rt = toc;
  ...
  for i = 1:length(result)
    for j = 1:length(result{i}{1})
      ...
      fprintf(fid, '%c\t', result{i}{6}{j});
      fprintf(fid, '%f\t', result{i}{7}{j});
      fprintf(fid, '%c\n', result{i}{8}{j});
    end
  end
  fclose(fid);
